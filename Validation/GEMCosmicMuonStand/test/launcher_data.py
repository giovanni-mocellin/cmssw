...
if __name__ == '__main__':
	...
	srcPath = os.path.abspath("launcher_data.py").split('gemcrs')[0]+'gemcrs/src/'
	pyhtonModulesPath = os.path.abspath("launcher_data.py").split('gemcrs')[0]+'gemcrs/src/Validation/GEMCosmicMuonStand/python/'
	runPath = os.path.abspath("launcher_data.py").split('gemcrs')[0] + 'gemcrs/src/Validation/GEMCosmicMuonStand/test/'
	resDirPath = os.path.abspath("launcher_data.py").split('gemcrs')[0]
	...
	runCommand = "cmsRun runGEMCosmicStandDB_reco.py inputFiles=file:/afs/cern.ch/user/d/dorney/public/v3Hack/run000000_Cosmic_CERNQC8_2018-11-09_chunk_321.dat localMode=True"
	...
	effoutDir = os.path.abspath("launcher_data.py").split('gemcrs')[0] + outDirName
	for i in range (0,30):
