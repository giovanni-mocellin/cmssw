...
void CSCMotherboard::matchALCTCLCT() {
  ...
  for (int bx_alct = 0; bx_alct < CSCConstants::MAX_ALCT_TBINS; bx_alct++) {
    ...
    // Do not consider LCT building in this BX if two GEM-CSC LCTs
    // were already found. This check should have no effect on the
    // regular LCT finding. It does play a role in the LCT finding
    // for GEM-CSC ILTs (in CSCGEMMotherboard)
    unsigned nLCTs = 0;
    // check each of the preferred combinations
    for (unsigned int mbx = 0; mbx < match_trig_window_size; mbx++) {
      for (int i = 0; i < CSCConstants::MAX_LCTS_PER_CSC and nLCTs < CSCConstants::MAX_LCTS_PER_CSC; i++) {
        if (allLCTs_(bx_alct, mbx, i).isValid()) nLCTs++;
      }
    }
